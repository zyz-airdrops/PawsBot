...
def is_latest_tg_version(user_agent: str) -> bool:
    tg_version = re.search(r'Telegram-Android/([\d\.]+)', user_agent)
    if tg_version:
        version = tg_version.group(1)
        return version in telegram_versions
    return False
